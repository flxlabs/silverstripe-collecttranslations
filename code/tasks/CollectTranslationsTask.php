...
class CollectTranslationsTask extends BuildTask {
    ...
    protected $description = 'Collects translations from template and code files.<br>
    	<table>
    		<tr>
    		...
    		<tr>
    			...
    			<td><a href="tasks/CustomTask?verbose=1">verbose=1</a></td>
    		</tr>
    		<tr>
    			<td><b>compare</b></td>
    			<td>[lang]</td>
    			<td>Compare to an existing language file (e.g. "de")</td>
    			<td><a href="tasks/CustomTask?compare=de">compare=de</a></td>
    		</tr>
 		</table>';
    ...
    function run($request) {
    	...
    	$arr = array();
    	$arr = array_merge_recursive($arr, self::processDir("/themes/", $verbose));
        $arr = array_merge_recursive($arr, self::processDir("/mysite/", $verbose));
		...
		foreach ($iterator as $file) {
		    ...
		    foreach (file($file) as $fli => $fl) {
		    	...
		    	if ($pos !== false) {
					...
					$splits = array_filter(mb_split(" ", $data));
		    		$path = array_filter(mb_split("\.", $splits[0]));
                    ...
                    if ($pos !== false) {
                        ...
                        if (mb_strpos($key, "$") !== false) {
                            ...
                            $path = array_filter(mb_split("\.", $key));
                        }
                    }
                }
